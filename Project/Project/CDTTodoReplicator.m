...
-(NSURL*)replicatorURL {
    ...
    //Using local CouchDB for testing purposes
    NSString *cleanURL = [NSString stringWithFormat:@"http://localhost:5984/%@", db_name];
    [self log:cleanURL];
    
    NSString *url = [NSString stringWithFormat:@"http://localhost:5984/%@", db_name];
...
-(void)sync
{
    NSLog(@"Synchronizing to CouchDB...");
    [self pullReplication];
    NSLog(@"Pull replication finished!");
    [self pushReplication];
    NSLog(@"Push replication finished!");
}
